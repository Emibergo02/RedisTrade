...
public class TargetGui extends AbstractTradeGui {


    public TargetGui(TradeGUI trade) {
        super(trade);
    }
    ...
    public void drawOrNotifyConfirm() {
        ...
        setItem(0, new SuppliedItem(() -> {
            ...
            trade.setTargetConfirm(!trade.isTargetConfirmed());
            return true;
        }));
    }
    ...
    public void drawOrNotifyMoney() {
        Optional.ofNullable(getItem(1)).ifPresentOrElse(Item::notifyWindows, () ->
                setItem(1, new SuppliedItem(() -> new ItemBuilder(Material.GOLD_NUGGET)
                    ...
                    new MoneySelectorGUI(this, trade.getTargetPrice(), clickEvent.getPlayer());
                    return false;
                })));
    ...
    public void drawRightSide() {
        for (int i = 0; i < trade.getTraderSideInfo().getItems().length; i++) {
            int finalI = i;
            int x = 8 - (i % 4);
            int y = i / 4 + 1;
            System.out.println("Drawing item at x: " + x + " y: " + y);
            ...
            for (int slotN = startingIndex; slotN < startingIndex + 4; slotN++) {
                setItem(slotN, new AbstractItemSlot() {
                    ...
                    public void updateItem(int slot, ItemStack item) {
                        int x = slot % 9;
                        int y = slot / 9;
                        trade.updateTargetItem(x, y, item);
                    }
                });
            }
        }
